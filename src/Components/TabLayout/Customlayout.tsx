...
import {
  getAllCases,
  UpdatecaseInfo,
} from "../../Service/AllCases/AllCaseService";
...
const CommonTabLayout: React.FC<CommonTabLayoutProps> = ({
  title,
  basePath,
  tabs,
}) => {
  ...
  const [SelectStatus, setSelectStatus] = React.useState<SelectOption>({
    label: "",
    value: "",
  });
  ...
  const init = async () => {
    const Status = await getChoiceDropdownOptions(
      "Status",
      constants.Listnames.CaseDetails
    );
